...
{	
	...
	if ( IsHarvesterAlive( fd_harvester.harvester ) )
		...
		print( "Wave aborted, Harvester destroyed" )
	...
	{
		if ( IsValid( tick ) && IsAlive( tick ) )
			tick.Die( svGlobal.worldspawn, svGlobal.worldspawn, { scriptType = DF_SKIP_DAMAGE_PROT | DF_SKIPS_DOOMED_STATE | DF_GIB, damageSourceId = eDamageSourceId.round_end } )
	}
}
...
bool function runWave( int waveIndex, bool shouldDoBuyTime )
{
	...
	if ( !IsHarvesterAlive( fd_harvester.harvester ) )
	{
		...
		// Unclassified attackers leave no ranked damage; keep the empty slots and report zero shares.
		float damageRatioScale = totalDamage > 0.0 ? 1.0 / totalDamage : 0.0
		...
		if ( FD_PlayersHaveRestartsLeft() )
		{
			...
			foreach ( entity player in GetPlayerArrayOfTeam( TEAM_MILITIA ) )
			{
				Remote_CallFunction_NonReplay( player, "ServerCallback_FD_DisplayHarvesterKiller", FD_GetNumRestartsLeft(), getHintForTypeId( highestDamageSource[0] ), highestDamageSource[0], highestDamage[0] * damageRatioScale, highestDamageSource[1], highestDamage[1] * damageRatioScale, highestDamageSource[2], highestDamage[2] * damageRatioScale )
		...
		else
		{
			...
			SetWinner( TEAM_IMC, "#FD_TOTAL_DEFEAT_HINT", "#FD_TOTAL_DEFEAT_HINT", false )
	...
	if ( isFinalWave() && IsHarvesterAlive( fd_harvester.harvester ) )
	{
		...
		else
		{
			...
			SetWinner( TEAM_MILITIA, "#FD_TOTAL_VICTORY_HINT", "#FD_TOTAL_VICTORY_HINT", false )
			return true
		}
...
void function OnHarvesterDamaged( entity harvester, var damageInfo )
{
	...
	if ( IsValid( weapon ) && HeavyArmorCriticalHitRequired( damageInfo ) && !attacker.IsTitan() ) //Small change since Grunts will do 0 damage with normal guns because Harvester uses heavy armor
	...
	*/
	...
	if ( harvester.GetShieldHealth() == 0 )
	{
		...
		if ( newHealth <= 0 )
		{
			...
			damageAmount = 0.0 // Preserve suppression through the final DamageInfo write below.
...
void function AddTurretSentry( entity turret )
{
	...
	else if ( player != null && player.GetTeam() == TEAM_IMC && IsAlive( turret ) )
		turret.Die( svGlobal.worldspawn, svGlobal.worldspawn, { scriptType = DF_SKIP_DAMAGE_PROT | DF_SKIPS_DOOMED_STATE | DF_GIB, damageSourceId = eDamageSourceId.round_end } ) //IMC Players shall not deploy turrets
}
...
void function ClearInvalidFDEntities()
{
	...
	{
		if ( IsValid( turret ) && IsAlive( turret ) && !IsValidPlayer( turret.GetBossPlayer() ) && turret.GetAISettingsName() == "npc_turret_sentry_burn_card_ap_fd" )
			turret.Die( svGlobal.worldspawn, svGlobal.worldspawn, { scriptType = DF_SKIP_DAMAGE_PROT | DF_SKIPS_DOOMED_STATE | DF_GIB, damageSourceId = eDamageSourceId.round_end } )
	}
}
...
void function FD_WaveCleanup()
{
	...
	foreach ( entity npc in GetNPCArray() ) //Turret Handling
	{
		...
		// Preserve child-entity teardown through the normal NPC death path.
